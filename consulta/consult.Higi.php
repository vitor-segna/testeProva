    ...
    <ul>
     ...
     <?php
        ...
        if (isset($_GET['acao']) && isset($_GET['id'])) {
            ...
            if ($acao == 'entrada') {
                $sql_update = "UPDATE higiene SET codigo = codigo + 1 WHERE idhigiene = $id";
                mysqli_query($conn, $sql_update);

                $sql_historico = "INSERT INTO movimentacoes_geral (id_produto, categoria_produto, tipo_movimentacao, quantidade, data_movimento) VALUES ($id, 'higiene', 'ENTRADA', 1, NOW())";
                mysqli_query($conn, $sql_historico);

            } elseif ($acao == 'saida') {
                $sql_update = "UPDATE higiene SET codigo = codigo - 1 WHERE idhigiene = $id AND codigo > 0";
                mysqli_query($conn, $sql_update);

                $sql_historico = "INSERT INTO movimentacoes_geral (id_produto, categoria_produto, tipo_movimentacao, quantidade, data_movimento) VALUES ($id, 'higiene', 'SAÍDA', 1, NOW())";
                mysqli_query($conn, $sql_historico);
            }
        }
