    ...
    <ul>
     ...
     <?php
        ...
        if (isset($_GET['acao']) && isset($_GET['id'])) {
            ...
            if ($acao == 'entrada') {
                $sql_update = "UPDATE alimentos SET peso = peso + 1 WHERE idalimentos = $id";
                mysqli_query($conn, $sql_update);

                $sql_historico = "INSERT INTO movimentacoes_geral (id_produto, categoria_produto, tipo_movimentacao, quantidade, data_movimento) VALUES ($id, 'alimento', 'ENTRADA', 1, NOW())";
                mysqli_query($conn, $sql_historico);

            } elseif ($acao == 'saida') {
                $sql_update = "UPDATE alimentos SET peso = peso - 1 WHERE idalimentos = $id AND peso > 0";
                mysqli_query($conn, $sql_update);

                $sql_historico = "INSERT INTO movimentacoes_geral (id_produto, categoria_produto, tipo_movimentacao, quantidade, data_movimento) VALUES ($id, 'alimento', 'SAÍDA', 1, NOW())";
                mysqli_query($conn, $sql_historico);
            }
        }
